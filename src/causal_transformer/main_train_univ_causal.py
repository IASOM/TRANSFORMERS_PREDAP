...
from univariate_transformer import (
    ...
    extract_model_params, load_and_evaluate_models, 
    setup_gpu_memory,create_model_directories, create_pandemic_waves_df, 
    load_and_preprocess_data, evaluate_univ_transformer, evaluate_causal_univ_transformer,
...
def main_univ_transformer(lookback, forecast, code,
    ...
    X, Y = data_preparation.prepare_causal_data(input_directory, code, LOOKBACK, FORECAST,covid_token=COVID_TOKEN, cutoff_date=cutoff_date,train = True, debug=True, univariate=True)
    ...
    # Build model
    model = build_model(
        (X.shape[0], X.shape[-1]),  # Input shape
        head_size=HEAD_SIZE,
        num_heads=NUM_HEADS,
        ff_dim=FF_DIM,
        num_transformer_blocks=NUM_TRANSFORMER_BLOCKS,
    ...
    # Train initial model
    MODEL_NAME = f'{code}_causal_transformer_{FORECAST}fh_{FF_DIM}ff_{LOOKBACK}lb_{LEARNING_RATE}initlr.keras'
    train_given_model_and_data(
        model, X, Y, 
        batch_size=batch_size,
    ...
    if evaluate_model:
        loss, mae, mse = evaluate_causal_univ_transformer(MODEL_NAME, input_directory, code, cutoff_date=cutoff_date,covid_token=COVID_TOKEN, MODEL_FOLDER=MODEL_FOLDER, df_waves=df_waves)
