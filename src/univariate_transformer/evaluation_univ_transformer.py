...
def evaluate_univ_transformer(model_name, input_directory, code,  cutoff_date, covid_token = False, MODEL_FOLDER='models_univariate_transformer', df_waves=None):
    ...
    date_list = data_preparation.extract_dates(input_directory, code, lookback, forecast, train=False, cutoff_date=cutoff_date)
...
def evaluate_causal_univ_transformer(model_name, input_directory, code,  cutoff_date, covid_token = False, MODEL_FOLDER='models_univariate_transformer', df_waves=None):
    print(f"\n--- Evaluating model: {model_name} ---")
